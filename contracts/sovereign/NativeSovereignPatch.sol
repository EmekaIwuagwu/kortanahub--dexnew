...
contract NativeSovereignPatch is ReentrancyGuard {
    ...
    // Manual DNR Deposit (to bypass the 2300 gas limit)
    function depositDNR() external payable {
        sync();
    }

    // Sell USDC to Buy DNR (Grow DNR Price) - UNBLOCKABLE
    function buyDNR(uint256 usdcAmountIn, uint256 minDnrOut, address to) external nonReentrant {
        ...
        sync(); // Refresh reserves before math
