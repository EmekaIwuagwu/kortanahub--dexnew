...
# [INSTITUTIONAL REGISTRY - KORTANA MAINNET]
...
# We use the verified functional pair for USDC.k
DNR_USDC_PAIR = "0x4251Bfe762EB0535a22C4653b4353f184A13eb4d"
...
class KortanaUSDCSimulator:
    def __init__(self, master_key: str):
        ...
        print(f"[SYSTEM] USDC.k SIMULATOR INITIALIZED | MASTER: {self.master.address}")
    ...
    def buy_usdc_k(self, amount_dnr):
        ...
        tx = self.router.functions.swapDNRForTokens(
            self.w3.to_checksum_address(DNR_USDC_PAIR), 0, self.master.address, int(time.time()) + 600
        ).build_transaction({
        ...
        return self.wait_for_success(self.w3.eth.send_raw_transaction(signed.raw_transaction if hasattr(signed, 'raw_transaction') else signed.rawTransaction), "DNR -> USDC.k")
    ...
    def sell_usdc_k(self, amount_usdc):
        ...
        tx = self.router.functions.swapTokensForDNR(
            self.w3.to_checksum_address(DNR_USDC_PAIR), self.w3.to_checksum_address(USDC_K_ADDRESS),
...
if __name__ == "__main__":
    ...
    sim = KortanaUSDCSimulator(MASTER_KEY)
    
    # Execute USDC.k Scenarios
    sim.buy_usdc_k(0.01)
    sim.sell_usdc_k(1.5)
    
    print("\n--- USDC.k VERIFICATION COMPLETE ---")
