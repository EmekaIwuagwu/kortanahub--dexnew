...
async function main() {
  ...
  const token = await ethers.getContractAt("@openzeppelin/contracts/token/ERC20/IERC20.sol:IERC20", wdnr, wallet);

  console.log(`Checking WDNR balance...`);
  const bal = await token.balanceOf(wallet.address);
  console.log(`Balance: ${ethers.formatUnits(bal, 18)} WDNR`);

  console.log(`Testing WDNR Transfer...`);
  const tx = await token.transfer(wallet.address, 1n, { type: 0, gasPrice: ethers.parseUnits("3", "gwei"), gasLimit: 500000 });
  console.log(`Tx: ${tx.hash}`);
  await tx.wait();
  console.log(`✅ WDNR Transfer WORKED!`);
}
